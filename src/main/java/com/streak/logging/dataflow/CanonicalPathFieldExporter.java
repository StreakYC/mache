 ...
 */
public class CanonicalPathFieldExporter implements FieldExporter {
    @Override
    public Object getFieldValue(JSONObject log) {
        ...
        for (int i = 0; i < logLines.length(); i++) {
            ...
            if (LogUtils.isLineHeadingFor(Constants.REQUEST_DATA_LOG_LINE, line)) {
                return LogUtils.extractParameter("canonicalPath", line);
            }
        }
    ...
    public TableFieldSchema getSchema() {
        return new TableFieldSchema().setName("canonicalPath").setType("STRING");
    }
}
